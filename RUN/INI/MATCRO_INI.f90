PROGRAM MATCRO
  ...
  CHARACTER*50 COUNTRY ! Calculating point [-]
  CHARACTER*50 REGION ! Calculating point [-]
  INTEGER STYR      ! Start year [Year]
  INTEGER FXYR      ! fixing year[Year]
  ...
  CHARACTER*5 SEASON  ! Time resolution of climate forcing [Second]
  ...
  CHARACTER*200 PHNFILE  ! Foot of input file name for atmospheric CO2
  CHARACTER*200 GDHh_FILE  ! Foot of input file name for atmospheric CO2
  CHARACTER*200 GDHm_FILE  ! Foot of input file name for atmospheric CO2
  CHARACTER*200 LONLAT_FILE  ! Foot of input file name for atmospheric CO2
  ...
  CHARACTER*200 LAImx_FILE   ! Output file name for PRM ← HI for final
  CHARACTER*200 WSH_FILE   ! Output file name for PRM ← HI for final
  CHARACTER*200 WSO_FILE   ! Output file name for PRM ← HI for final
  CHARACTER*200 WAR_FILE   ! Output file name for PRM ← HI for final
  CHARACTER*200 WST_FILE   ! Output file name for PRM ← HI for final
  CHARACTER*200 WLF_FILE   ! Output file name for PRM ← HI for final
  CHARACTER*200 WRT_FILE   ! Output file name for PRM ← HI for final
  CHARACTER*200 WDL_FILE   ! Output file name for PRM ← HI for final
  CHARACTER*200 GPP_FILE   ! Output file name for PRM ← HI for final
  CHARACTER*200 TSP_FILE   ! Output file name for PRM ← HI for final
  CHARACTER*200 RSP_FILE   ! Output file name for PRM ← HI for final
  CHARACTER*200 SLN_FILE   ! Output file name for PRM ← HI for final
  CHARACTER*200 LAI_FILE   ! Output file name for PRM ← HI for final
  CHARACTER*200 DVS_FILE   ! Output file name for PRM ← HI for final
  ...
  CHARACTER*200 SMPL_HEAD
  CHARACTER*5   SMPL_FOOT
  CHARACTER*10  BFGS_DIR
  ...
  !! [INTERNAL VARIABLE] !!
  !! !!!!!!!!!!!!!!!!!!! !!
  CHARACTER*5 IRR_NAME ! IRRIGATION name ir or rf
...
!  [OUTPUT]
  ...
  REAL*8,ALLOCATABLE:: OWSH(:,:,:)   ! CDI - HI [-(%)]
  REAL*8,ALLOCATABLE:: OWSO(:,:,:)   ! CDI - HI [-(%)]
  REAL*8,ALLOCATABLE:: OWAR(:,:,:)   ! CDI - HI [-(%)]
  REAL*8,ALLOCATABLE:: OWST(:,:,:)   ! CDI - HI [-(%)]
  REAL*8,ALLOCATABLE:: OWLF(:,:,:)   ! CDI - HI [-(%)]
  REAL*8,ALLOCATABLE:: OWRT(:,:,:)   ! CDI - HI [-(%)]
  REAL*8,ALLOCATABLE:: OWDL(:,:,:)   ! CDI - HI [-(%)]

  REAL*8,ALLOCATABLE:: ODVS(:,:,:)   ! Output DVS
  REAL*8,ALLOCATABLE:: OGPP(:,:,:)   ! Output GPP
  REAL*8,ALLOCATABLE:: OTSP(:,:,:)   ! Output TSP
  REAL*8,ALLOCATABLE:: ORSP(:,:,:)   ! Output RSP
  REAL*8,ALLOCATABLE:: OSLN(:,:,:)   ! Output SLN
  REAL*8,ALLOCATABLE:: OLAI(:,:,:)   ! Output LAI
...
!  [FUNCTION]
  ...
  !!  <READ SETTING FILE> !!
  !! !!!!!!!!!!!!!!!!!!!! !!
  CALL RDSET(MODE,COUNTRY,REGION,STYR,ENYR,STDOY,ENDOY,POINTLON,POINTLAT,TRES,clmTRES,SEASON,IRR_NAME,IRR,CRP_NAME,CRP_FILE,&
             CNFILE,PLT_FILE,YLD_FILE,PRM_FILE,LAImx_FILE,WSH_FILE,WSO_FILE,WAR_FILE,WST_FILE,WLF_FILE,WRT_FILE,WDL_FILE,&
             CLIM_HEAD,CLIM_FOOT)
...
!!!< Allocate >
  ...
  ALLOCATE(OWSH(NLON,NLAT,ENYR-STYR+1),OWSO(NLON,NLAT,ENYR-STYR+1),OWAR(NLON,NLAT,ENYR-STYR+1),OWST(NLON,NLAT,ENYR-STYR+1),OWLF(NLON,NLAT,ENYR-STYR+1),OWRT(NLON,NLAT,ENYR-STYR+1),OWDL(NLON,NLAT,ENYR-STYR+1))
  ALLOCATE(OGPP(NLON,NLAT,(ENDOY-STDOY+1)),ODVS(NLON,NLAT,(ENDOY-STDOY+1)))
  ALLOCATE(OLAI(NLON,NLAT,(ENDOY-STDOY+1)),OSLN(NLON,NLAT,(ENDOY-STDOY+1)))
  ALLOCATE(OTSP(NLON,NLAT,(ENDOY-STDOY+1)),ORSP(NLON,NLAT,(ENDOY-STDOY+1)))
  !! [CROP PARAMETER FILE] !!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !!!!! READING INPUT PHENOLOGICAL DATA !!!!!
  !GDHh_FILE ="/home/tnakagawa/MATCRO_JAPAN/GDD/POINT/FINAL/GDDh/GDDh_"//trim(POINT)//".txt"
  !GDHm_FILE ="/home/tnakagawa/MATCRO_JAPAN/GDD/POINT/FINAL/GDDm/GDDm_"//trim(POINT)//".txt"
  LONLAT_FILE="/home/tnakagawa/MATCRO_ASIA/INPUT/COORDINATE/"//trim(COUNTRY)//'/'//trim(REGION)//'_'//trim(IRR_NAME)//"_lonlat.csv"
  print *, trim(LONLAT_FILE)
  ...
  THcrit=37.D0
  !SLNY1=1.D0
  SLNY1=0.7742242627D0

  GPP_FILE="./GPP/"//TRIM(COUNTRY)//'/'//TRIM(REGION)//"/GPP_"//TRIM(REGION)//"_"
  TSP_FILE="./TSP/"//TRIM(COUNTRY)//'/'//TRIM(REGION)//"/TSP_"//TRIM(REGION)//"_"
  RSP_FILE="./RSP/"//TRIM(COUNTRY)//'/'//TRIM(REGION)//"/RSP_"//TRIM(REGION)//"_"
  SLN_FILE="./SLN/"//TRIM(COUNTRY)//'/'//TRIM(REGION)//"/SLN_"//TRIM(REGION)//"_"
  LAI_FILE="./LAI/"//TRIM(COUNTRY)//'/'//TRIM(REGION)//"/LAI_"//TRIM(REGION)//"_"
  DVS_FILE="./DVS/"//TRIM(COUNTRY)//'/'//TRIM(REGION)//"/DVS_"//TRIM(REGION)//"_"
  ...
  DO YEAR = STYR,ENYR
   ...
   IF(MODE=="ONE")THEN
      CALL RDCLIM(INTMN,CLIM_HEAD,COUNTRY,REGION,'tmin ',IRR_NAME,SEASON,YEAR,CLIM_FOOT,NLON,NLAT,365,FILE_CHECK)
      !print *,"TMN OK"
      CALL RDCLIM(INTMX,CLIM_HEAD,COUNTRY,REGION,'tmax ',IRR_NAME,SEASON,YEAR,CLIM_FOOT,NLON,NLAT,365,FILE_CHECK)
      !print *,"TMX OK"
      CALL RDCLIM(INPRC,CLIM_HEAD,COUNTRY,REGION,'prc  ',IRR_NAME,SEASON,YEAR,CLIM_FOOT,NLON,NLAT,365,FILE_CHECK)
      !print *,"PRC OK"
      CALL RDCLIM(INRSD,CLIM_HEAD,COUNTRY,REGION,'rsd  ',IRR_NAME,SEASON,YEAR,CLIM_FOOT,NLON,NLAT,365,FILE_CHECK)
      !print *,"RSD OK"
      CALL RDCLIM(INSHM,CLIM_HEAD,COUNTRY,REGION,'shm  ',IRR_NAME,SEASON,YEAR,CLIM_FOOT,NLON,NLAT,365,FILE_CHECK)
      !print *,"SHM OK"
      CALL RDCLIM(INWND,CLIM_HEAD,COUNTRY,REGION,'wnd  ',IRR_NAME,SEASON,YEAR,CLIM_FOOT,NLON,NLAT,365,FILE_CHECK)
      !print *,"WND OK"
      CALL RDCLIM(INPRS,CLIM_HEAD,COUNTRY,REGION,'prs  ',IRR_NAME,SEASON,YEAR,CLIM_FOOT,NLON,NLAT,365,FILE_CHECK)
      !print *,"PRS OK"
      INOZN=0.0D0
      
      CALL RDCO2_NFERT(CO2PPM,BUF(:,:),CNFILE,YEAR)           !CO2
      CALL RDCO2_NFERT(BUF(:,:),NFERT(NLON,NLAT),CNFILE,YEAR) !Nitrogen
   ...
   PLTDOY(NLON,NLAT)=150
   !CALL RDPHN(GDHm(NLON,NLAT),BUF,GDHm_FILE,YEAR)
   GDHm(NLON,NLAT)=3000.D0 - 1000.D0 / (1960-1896) * (YEAR-1896)
   GDHm(NLON,NLAT)=max(GDHm(NLON,NLAT),2000.D0)
   !hDVS = 0.7D0
   !CALL RDPHN(GDHh(NLON,NLAT),BUF,GDHh_FILE,YEAR)
   !hDVS = GDHh(NLON,NLAT) / GDHm(NLON,NLAT)
   !hDVS=0.5D0 + 0.1D0 / (1960-1896) * (YEAR-1896)
   !hDVS = min(hDVS, 0.7D0)
   ...
   !SLNYMN = 3.D0 - 1.0D0 / (1960-1896) * (YEAR-1896)
   !SLNYMN = max(SLNYMN,2.D0)
   !SLNYMN  = 2.0D0
   !SLNY1   = 1.0D0
   !LEFY0  = 0.6D0 - 0.2D0 / (1960-1896) * (YEAR-1896)
   !LEFY0  = max(LEFY0,0.4D0)
   !LEFY1  = LEFY0
   
   !print *, "LEFY0", LEFY0
 
   CALL RDLONLAT(POINTLON,POINTLAT,YEAR,LONLAT_FILE) ! fix if would like to see radiation effect 
   !POINTLON=126.D0 ! test Soeul longitude
   !POINTLAT=37.5D0 ! test Soeul latitude
...
!                    print *,GDHm(ILON,ILAT,1,GRW),PLTDOY(ILON,ILAT,1,GRW),LON,LAT
                    ...
                    IF(LND(ILON,ILAT) > 0.D0 .AND. NFERT(ILON,ILAT) > -1.D0 .AND. &
                       ...
                       DO IHOUR=1,86400/TRES
                          ...
                          !! Crop simulation
                          CALL CROP(YLD(ILON,ILAT,YEAR-STYR+1),PRM(ILON,ILAT,YEAR-STYR+1),OCDI(NLON,NLAT,YEAR-STYR+1),OWSH(NLON,NLAT,YEAR-STYR+1),OWSO(NLON,NLAT,YEAR-STYR+1),OWAR(NLON,NLAT,YEAR-STYR+1),OWST(NLON,NLAT,YEAR-STYR+1),OWLF(NLON,NLAT,YEAR-STYR+1),OWRT(NLON,NLAT,YEAR-STYR+1),OWDL(NLON,NLAT,YEAR-STYR+1),&
                          ...
                          OGPP(ILON,ILAT,DOY)=MAX(GPP,OGPP(ILON,ILAT,DOY))
                          OTSP(ILON,ILAT,DOY)=MAX(TSP,OTSP(ILON,ILAT,DOY))
                          ORSP(ILON,ILAT,DOY)=MAX(RSP,ORSP(ILON,ILAT,DOY))
                          OLAI(ILON,ILAT,DOY)=MAX(LAI,OLAI(ILON,ILAT,DOY))
                          OSLN(ILON,ILAT,DOY)=MAX(SLN,OSLN(ILON,ILAT,DOY))
                          ODVS(ILON,ILAT,DOY)=MAX(DVS,ODVS(ILON,ILAT,DOY))
                        END DO !HOUR
               ...
               END DO !ILON
               !print *, YEAR
               CALL WRCSV_GPP(YEAR,STDOY,ENDOY,GPP_FILE,OGPP(NLON,NLAT,:),ODVS(NLON,NLAT,:),NLON,NLAT,TRES) !GPP
               CALL WRCSV_GPP(YEAR,STDOY,ENDOY,TSP_FILE,OTSP(NLON,NLAT,:),ODVS(NLON,NLAT,:),NLON,NLAT,TRES) !GPP
               CALL WRCSV_GPP(YEAR,STDOY,ENDOY,RSP_FILE,ORSP(NLON,NLAT,:),ODVS(NLON,NLAT,:),NLON,NLAT,TRES) !GPP
               CALL WRCSV_GPP(YEAR,STDOY,ENDOY,LAI_FILE,OLAI(NLON,NLAT,:),ODVS(NLON,NLAT,:),NLON,NLAT,TRES) !LAI
               CALL WRCSV_GPP(YEAR,STDOY,ENDOY,SLN_FILE,OSLN(NLON,NLAT,:),ODVS(NLON,NLAT,:),NLON,NLAT,TRES) !SLN
            END DO ! YEAR
      ...
      CALL WRTXT_YLD(STYR,ENYR,LAImx_FILE,LAImx(NLON,NLAT,:),NLON,NLAT) !LAI
      CALL WRTXT_YLD(STYR,ENYR,WSH_FILE,OWSH(NLON,NLAT,:),NLON,NLAT) !WSH
      CALL WRTXT_YLD(STYR,ENYR,WSO_FILE,OWSO(NLON,NLAT,:),NLON,NLAT) !WSO
      CALL WRTXT_YLD(STYR,ENYR,WAR_FILE,OWAR(NLON,NLAT,:),NLON,NLAT) !WAR
      CALL WRTXT_YLD(STYR,ENYR,WST_FILE,OWST(NLON,NLAT,:),NLON,NLAT) !WST
      CALL WRTXT_YLD(STYR,ENYR,WLF_FILE,OWLF(NLON,NLAT,:),NLON,NLAT) !WLF
      CALL WRTXT_YLD(STYR,ENYR,WRT_FILE,OWRT(NLON,NLAT,:),NLON,NLAT) !WRT
      CALL WRTXT_YLD(STYR,ENYR,WDL_FILE,OWDL(NLON,NLAT,:),NLON,NLAT) !WDL 
